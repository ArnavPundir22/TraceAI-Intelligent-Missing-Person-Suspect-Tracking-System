...
class StreamProcessor:
    ...
    def __init__(self, camera_id: int, stream_url: str, zone: str = "Unknown"):
        ...
        # Timeline tracking: {person_id: {"entered_at": datetime, "last_seen": datetime, "snapshot": path}}
        self._active_timelines: Dict[int, dict] = {}
        self._timeline_timeout = 30.0  # seconds of inactivity before closing a timeline event
    ...
    async def stop(self):
        ...
        if self._task:
            ...
            try:
                ...
                pass
        # Flush all open timeline events before shutdown
        await self._flush_all_timelines()
    ...
    async def _process_loop(self):
        # Support webcam device indices: "0", "1", etc.
        ...
        while self.running:
            ...
            det_records = []
            for track in tracks:
                # Pass original BGR frame for InsightFace face extraction
                det_record = await self._process_track(frame, track)
                if det_record:
    ...
    async def _process_track(self, frame: np.ndarray, track) -> Optional[dict]:
        ...
        # Generate face embedding from the person crop
        # InsightFace will detect the face within the person bounding box
        embedding = await embedding_service.get_embedding(crop)

        # Identity matching (only possible if a face was found)
        person_id = None
        reid_score = 0.0
        is_watchlist_hit = False

        if embedding is not None and self._embeddings_cache:
        ...
        # Record detection (always — even without face embedding)
        now = datetime.utcnow()
        snapshot_str = str(snapshot_path) if snapshot_path else None
        face_emb_list = embedding.tolist() if embedding is not None else None
        ...
        # Update timeline tracking for identified persons
        if person_id:
            await self._update_timeline(person_id, now, snapshot_str)
    ...
    # Timeline Management
    # ------------------------------------------------------------------
    async def _update_timeline(self, person_id: int, timestamp: datetime, snapshot_path: Optional[str]):
        """Track per-person dwell time; finalize events after inactivity."""
        # First, finalize any stale timelines (person left camera view)
        stale_ids = []
        for pid, tl in self._active_timelines.items():
            elapsed = (timestamp - tl["last_seen"]).total_seconds()
            if elapsed > self._timeline_timeout:
                stale_ids.append(pid)
        for pid in stale_ids:
            await self._finalize_timeline(pid)

        # Update or start timeline for this person
        if person_id in self._active_timelines:
            self._active_timelines[person_id]["last_seen"] = timestamp
            if snapshot_path:
                self._active_timelines[person_id]["snapshot"] = snapshot_path
        else:
            self._active_timelines[person_id] = {
                "entered_at": timestamp,
                "last_seen": timestamp,
                "snapshot": snapshot_path,
            }

    async def _finalize_timeline(self, person_id: int):
        """Write a completed TimelineEvent to the database."""
        tl = self._active_timelines.pop(person_id, None)
        if not tl:
            return
        entered = tl["entered_at"]
        exited = tl["last_seen"]
        ...
        async with AsyncSessionLocal() as db:
            event = TimelineEvent(
                ...
                confidence=0.85,
                snapshot_path=tl.get("snapshot"),
            )
            db.add(event)
            await db.commit()
        logger.debug(f"[CAM-{self.camera_id}] Timeline event finalized for person {person_id}: {duration:.1f}s")

    async def _flush_all_timelines(self):
        """Finalize all open timeline events (called on stream stop)."""
        for pid in list(self._active_timelines.keys()):
            await self._finalize_timeline(pid)
