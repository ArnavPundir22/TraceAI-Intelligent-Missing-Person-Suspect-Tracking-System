...
async def lifespan(app: FastAPI):
    ...
    # Demo seeding disabled — start clean for live operation
    # To re-enable demo data, uncomment the next line:
    # await _seed_demo_data()
    logger.info("✓ Ready (clean database — no demo data)")
...
# ---------------------------------------------------------------------------
# Static Files (Frontend + Data Assets)
# ---------------------------------------------------------------------------
FRONTEND_DIR = Path(__file__).resolve().parent.parent.parent / "frontend"
DATA_DIR = Path(__file__).resolve().parent.parent / "data"

# Serve detection snapshots and uploaded face images
if (DATA_DIR / "snapshots").exists():
    app.mount("/data/snapshots", StaticFiles(directory=str(DATA_DIR / "snapshots")), name="snapshots")
if (DATA_DIR / "uploads").exists():
    app.mount("/data/uploads", StaticFiles(directory=str(DATA_DIR / "uploads")), name="uploads")
...
if FRONTEND_DIR.exists():
    ...
    async def serve_frontend(path: str):
        # Serve actual files if they exist (e.g. favicon, robots.txt)
        candidate = FRONTEND_DIR / path
        if candidate.is_file():
            return FileResponse(str(candidate))
        # Fallback: SPA — always return index.html for client-side routing
        index = FRONTEND_DIR / "index.html"
        if index.exists():
            return FileResponse(str(index))
