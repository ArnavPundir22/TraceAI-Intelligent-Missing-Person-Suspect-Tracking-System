(() => {
        ...
        ${state.cameras.map((camera) => `
          <article class="person-card ${camera.id === state.selectedCameraId ? 'suspect' : ''}" data-action="select-camera" data-camera-id="${camera.id}">
            ...
            <div class="flex gap-2">
              ...
              <button class="btn btn-danger btn-sm" data-action="delete-camera" data-camera-id="${camera.id}" title="Delete camera">✕</button>
            </div>
          </article>
      ...
      <section class="card">
        ...
        <div class="alert-feed" style="max-height:none;">
          ${state.alerts.length ? state.alerts.map((alert) => {
            const snapUrl = getSnapshotUrl(alert.snapshot_path);
            return `
            <div class="alert-item ${alert.severity}">
              <span class="alert-dot ${alert.severity}"></span>
              ${snapUrl ? `<img src="${snapUrl}" alt="alert snapshot" style="width:48px;height:48px;border-radius:6px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;" onerror="this.style.display='none'" />` : ''}
              <div style="flex:1;">
  ...
  function renderAlertItem(alert) {
    ...
    return `
      <div class="alert-item ${alert.severity}" data-action="goto-page" data-page="alerts">
        <span class="alert-dot ${alert.severity}"></span>
        ${snapUrl ? `<img src="${snapUrl}" alt="alert snapshot" style="width:42px;height:42px;border-radius:6px;object-fit:cover;border:1px solid var(--border);flex-shrink:0;" onerror="this.style.display='none'" />` : ''}
        <div style="flex:1;">
          <div class="alert-title">${escapeHtml(alert.title)}</div>
          <div class="alert-msg">${escapeHtml(alert.message)}</div>
        </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            ${detections.map((detection) => {
              const snapUrl = getSnapshotUrl(detection.snapshot_path);
              return `
              <tr>
                <td>${snapUrl ? `<img src="${snapUrl}" alt="snapshot" style="width:48px;height:48px;border-radius:6px;object-fit:cover;border:1px solid var(--border);" onerror="this.style.display='none'" />` : '<span class="text-muted">—</span>'}</td>
  ...
  async function handleDocumentSubmit(event) {
    ...
    if (form.id === 'edit-person-form') {
      event.preventDefault();
      await submitEditPerson(form);
    }
  ...
  async function handleDocumentChange(event) {
    ...
    if (target.id === 'person-face-photo') {
      const label = document.getElementById('person-photo-label');
      if (label) label.textContent = target.files?.[0]?.name || 'Clear frontal photo for best face detection results. Sketches also accepted.';
    }
  ...
  async function handleDocumentClick(event) {
    ...
    if (action === 'delete-camera') {
      const cameraId = Number(target.dataset.cameraId);
      const camera = state.cameras.find((c) => c.id === cameraId);
      openModal('Delete Camera', `
        <div class="empty-state" style="padding:20px;">
          ${alertIcon()}
          <div style="margin-top:12px;font-size:14px;color:var(--text-1);font-weight:600;">Are you sure you want to delete "${escapeHtml(camera?.name || 'this camera')}"?</div>
          <div class="text-muted" style="margin-top:8px;">This will stop any active stream and remove all camera configuration. Detection records will be preserved.</div>
        </div>
        <div class="flex gap-2 justify-between" style="margin-top:16px;">
          ...
          <button type="button" class="btn btn-danger" data-action="confirm-delete-camera" data-camera-id="${cameraId}">Delete Camera</button>
        </div>
      `);
      return;
    }

    if (action === 'confirm-delete-camera') {
      const cameraId = Number(target.dataset.cameraId);
  ...
  async function submitAddPerson(form) {
    ...
    try {
      await api.postForm('/persons/', formData);
      closeModal();
      await refreshAllData();
      showToast('Person registered', `${name} has been added to the registry.`);
    } catch (error) {
      showToast('Registration failed', formatError(error), 'error');
    }
  }
  ...
  async function deleteCamera(cameraId) {
    try {
      await api.del(`/cameras/${cameraId}`);
      closeModal();
      await refreshAllData();
      showToast('Camera deleted', 'Camera has been removed from the network.');
    } catch (error) {
      showToast('Delete failed', formatError(error), 'error');
    }
  }

  async function submitEditPerson(form) {
    const personId = Number(form.dataset.personId);
    const formData = new FormData(form);
    const payload = {};
    ...
    try {
      ...
      closeModal();
      await refreshAllData();
      showToast('Profile updated', `${payload.name || 'Person'} profile has been updated.`);
    } catch (error) {
      showToast('Update failed', formatError(error), 'error');
    }
  }
  ...
  function openAddPersonModal() {
    openModal('Add Person', `
      <form id="add-person-form" enctype="multipart/form-data">
        <div class="form-group">
          <label for="person-face-photo">Face Photo</label>
          <label class="search-upload-box" for="person-face-photo" style="margin-bottom:0;">
            <div class="search-upload-icon">${uploadIcon()}</div>
            <div class="upload-title">Drop or choose a face photo</div>
            <div class="upload-sub" id="person-photo-label">Clear frontal photo for best face detection results. Sketches also accepted.</div>
          </label>
          <input id="person-face-photo" name="face_photo" type="file" accept="image/*" hidden />
        </div>
        <div class="grid-2">
          <div class="form-group">
            <label for="person-name">Name <span style="color:#ef4444;">*</span></label>
            <input id="person-name" name="name" type="text" required placeholder="Full name of the person" />
          </div>
          <div class="form-group">
            <label for="person-alias">Alias / Known As</label>
            <input id="person-alias" name="alias" type="text" placeholder="Nickname, alias, etc." />
          </div>
        </div>
        <div class="grid-2">
          <div class="form-group">
            <label for="person-age">Age</label>
            <input id="person-age" name="age" type="number" min="0" placeholder="Approximate age" />
          </div>
          <div class="form-group">
            <label for="person-watchlist">Watchlist Status <span style="color:#ef4444;">*</span></label>
        ...
        <div class="form-group">
          <label for="person-description">Description <span class="text-muted" style="font-weight:400;">(optional — use when no photo is available)</span></label>
          <textarea id="person-description" name="description" placeholder="Height, build, hair, clothing, distinguishing marks…"></textarea>
        </div>
    ...
    openModal('Add Camera', `
      <form id="add-camera-form">
        ...
        <div class="form-group">
          ...
          <input id="camera-stream-url" name="stream_url" type="text" placeholder="0 (webcam) / rtsp://... / /path/to/video.mp4" />
          <div class="text-muted" style="font-size:11px;margin-top:4px;">Enter <b>0</b> for default webcam, <b>1</b> for secondary camera, or an RTSP/file URL.</div>
        </div>
        <div class="grid-2">
          <div class="form-group">
  ...
  function openPersonModal(personId) {
    ...
    openModal(person.name, `
      ...
      <div class="flex gap-2 justify-between mt-4">
        ...
        <div class="flex gap-2">
          <button type="button" class="btn btn-outline" data-action="edit-person" data-person-id="${person.id}">Edit Profile</button>
  ...
  function openEditPersonModal(personId) {
    ...
    openModal(`Edit — ${person.name}`, `
      <form id="edit-person-form" data-person-id="${person.id}">
        <div class="grid-2">
          <div class="form-group">
            <label for="edit-person-name">Name</label>
            <input id="edit-person-name" name="name" type="text" value="${escapeHtml(person.name)}" required />
          </div>
          <div class="form-group">
            <label for="edit-person-alias">Alias</label>
            <input id="edit-person-alias" name="alias" type="text" value="${escapeHtml(person.alias || '')}" placeholder="Nickname or alias" />
          </div>
        </div>
        <div class="grid-2">
          <div class="form-group">
            <label for="edit-person-age">Age</label>
            <input id="edit-person-age" name="age" type="number" min="0" value="${person.age ?? ''}" />
          </div>
          <div class="form-group">
            <label for="edit-person-watchlist">Watchlist Status</label>
            <select id="edit-person-watchlist" name="watchlist_status">
              ${['none', 'missing', 'suspect', 'person_of_interest'].map((s) => `
                <option value="${s}" ${person.watchlist_status === s ? 'selected' : ''}>${formatWatchlistLabel(s)}</option>
              `).join('')}
            </select>
          </div>
        </div>
        <div class="form-group">
          <label for="edit-person-desc">Description</label>
          <textarea id="edit-person-desc" name="description" placeholder="Physical description, clothing, distinguishing marks…">${escapeHtml(person.description || '')}</textarea>
        </div>
        <div class="flex gap-2 justify-between">
          <button type="button" class="btn btn-outline" data-action="close-modal">Cancel</button>
  ...
  function getSnapshotUrl(snapshotPath) {
    if (!snapshotPath) return null;
    // Extract just the filename from the absolute path
    const parts = snapshotPath.replace(/\\/g, '/').split('/');
    const filename = parts[parts.length - 1];
    if (!filename) return null;
    // Determine if it's from snapshots or uploads
    if (snapshotPath.includes('/uploads/')) {
